# Addin entry point registered in inst/rstudio/addins.dcf
#' @export
check_chunk_addin <- function() check_chunk()


#' Validate a selected R code chunk for common errors
#'
#' Checks: syntax, undefined global variables, invalid function argument names,
#' bad file/directory paths, and missing checkpoint .rds files.
#'
#' @param code  R code as a character string. NULL (default) reads the current
#'   selection from the RStudio editor.
#' @param env   Environment to resolve symbols against. Default: .GlobalEnv.
#' @param show_ok  Print OK confirmations in addition to errors/warnings.
#' @export
check_chunk <- function(code = NULL, env = .GlobalEnv, show_ok = TRUE) {
  ...
  if (!is.null(fn_wrap)) {
    ...
    built_ins <- c(
      ...
      "match.arg","missing","Sys.time","Sys.sleep",
  ...
  .cc_walk(parsed, function(node) {
    ...
    if (length(bad) > 0L)
      .add("ERROR", sprintf(
        "`%s()` — invalid arg(s): %s\n        Valid: %s",
        fn_label, paste(bad, collapse = ", "), paste(fmls, collapse = ", ")
      ))
    else if (show_ok)
  ...
  .cc_walk(parsed, function(node) {
    ...
    if (grepl("[*?]", evaluated) || !grepl("[./]", evaluated)) return()
  ...
  if (is.null(op)) {
    ...
    .cc_walk(parsed, function(node) {
      ...
      if (file.exists(cp_path)) {
        if (show_ok) .add("OK", sprintf('Checkpoint "%s" exists.', name_arg))
      } else {
        .add("WARNING", sprintf(
