...
run_kegg_enrichment <- function(
  ...
  convert_to_entrez <- function(gene_set) {
    mapped <- clusterProfiler::bitr(names(gene_set),
                                    ...
                                    toType   = "ENTREZID",
                                    OrgDb    = org_db)
    if (nrow(mapped) == 0) { warning("No ENTREZ IDs mapped. Returning NULL."); return(NULL) }
    gl_entrez <- gene_set[names(gene_set) %in% mapped$SYMBOL]
    gl_entrez <- gl_entrez[mapped$SYMBOL]
    names(gl_entrez) <- mapped$ENTREZID
    sort(gl_entrez, decreasing = TRUE)
  }

  gene_lists_entrez <- lapply(gene_lists, function(gl) {
    tryCatch(convert_to_entrez(gl),
             error = function(e) { message("ENTREZ conversion failed: ", e$message); NULL })
  })
