...
ms_genes <- c("Cx3cl1","PSD95","Syn1","Gabra1", "Gabra2","Snca","Mapt","Fos","Jun","Nefl","Nefm","Nefh")
gene_list <- ms_genes
# Initialize an empty dataframe to store the final result
final_df <- data.frame(gene = factor(), avg_log2FC = numeric(), p_val_adj = numeric())
...
sheet_names <- names(cluster_list)
...
if (!is.null(names(cluster_list))) {
  ...
  final_df <- bind_rows(lapply(names(cluster_list), function(name) process_df(cluster_list[[name]], name)))
...
final_df$gene <- as.factor(final_df$gene)
...
genes_presentes <- unique(final_df$gene)

subset_df <- final_df %>% 
  dplyr::select(gene, avg_log2FC, cluster_index) %>% 
